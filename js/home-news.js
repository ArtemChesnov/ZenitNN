...
function enableSlider() {
  ...
  sliderWrapper.addEventListener("wheel", (e) => {
    ...
    if (!isScrollable) return; // если нечего скроллить по горизонтали — ничего не делаем

    if (Math.abs(e.deltaY) > Math.abs(e.deltaX)) {
      // если движение больше по вертикали, чем по горизонтали — не мешаем скроллу страницы
      return;
    }

    e.preventDefault(); // только если скроллим влево/вправо
  ...
  sliderWrapper.addEventListener("scroll", () => {
    ...
    scrollTimeout = setTimeout(() => {
      snapToClosestSlide();
    }, 120); // сработает через 120мс после окончания скролла
  });
